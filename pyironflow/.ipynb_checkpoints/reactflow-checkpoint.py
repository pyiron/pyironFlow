...
def get_node_dict(node, id_num, key=None, macro=False, node_list=[]):
    node_width = 240
    ...
    #    distinct = "_" + str((node_list.count(label)-1))
    #else:
    #    distinct = ""                    
    
    if macro:
        node_type = 'customMacroNode'
    else:
...
def get_macro_child_node_dict(node, parentNode, id_num, key=None):
    ...
    label = node.label
    #label = f'{parentNode.label}_{node.label}'
...
def get_ghost_node_dict(parent_node, source, num):
    ...
    return {
        ...
        'targetPosition': 'left',
        'sourcePosition': 'right',
        'parentId': parent_node.label,
        'extent': 'parent',
    }

    
def get_nodes(wf, expanded_macros):
    nodes = []
    for i, (k, v) in enumerate(wf.children.items()):
        if (get_node_instance_type(v) == "macro" and k in expanded_macros):
            nodes.append(get_macro_node_dict(v, id_num=i, key=k))
            #nodes.append(get_node_dict(v, id_num=i, key=k))
            for child in list(v):
                nodes.append(get_macro_child_node_dict(child, v, id_num=i, key=k))
        ...
        elif (get_node_instance_type(v) == "function"):
            nodes.append(get_node_dict(v, id_num=i, key=k, macro=False))
    return nodes
...
def get_edges(wf, expanded_macros):
    edges = []
    for ic, (out, inp) in enumerate(wf.graph_as_dict["edges"]["data"].keys()):
        ...
        edge_dict["id"] = out_node + '__' + inp_node + '__' + inp_port
...
class PyironFlowWidget:
    def __init__(self, root_path='../pyiron_nodes', wf: Workflow = Workflow(label='workflow'), log=None, out_widget=None):
        self.log = log
    ...
    def get_workflow(self):
        ...
        for dict_node in dict_nodes:
            if ((dict_node['type'] != 'macroSubNode') and (dict_node['type'] != 'ghostNode')):
